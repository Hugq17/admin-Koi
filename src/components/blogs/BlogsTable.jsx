...
const BlogsTable = () => {
	...
	const confirmDelete = async () => {
		try {
			await axios.delete(`https://koi-care-at-home-server-h3fyedfeeecdg7fh.southeastasia-01.azurewebsites.net/api/blogs/delete/${selectedBlog.id}`);
	...
	const saveCreate = async (newBlog) => {
		try {
			const response = await axios.post('https://koi-care-at-home-server-h3fyedfeeecdg7fh.southeastasia-01.azurewebsites.net/api/blogs/create', newBlog);
			setBlogs([response.data, ...blogs]);
		...
		<motion.div className='bg-gray-800 bg-opacity-50 backdrop-blur-md shadow-lg rounded-xl p-6 border border-gray-700 mb-8'>
			...
			<div className='overflow-x-auto'>
				<table className='min-w-full divide-y divide-gray-700'>
					<thead>
						<tr>
							<th className='px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider whitespace-nowrap'>Tiêu đề</th>
							...
							<th className='px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider whitespace-nowrap'>Ngày tạo</th>
							<th className='px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider whitespace-nowrap'>Tác giả</th>
